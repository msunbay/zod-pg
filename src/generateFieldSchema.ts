...
import { getEnumConstantName, singularPascalCase } from './utils';
...
export const getEnums = (
  ...
  for (const colName in enumConstraints) {
    ...
    enumLiterals.push(
      `export const ${enumName} = ${JSON.stringify(enumConstraints[colName])} as const;`
    );

    enumTypes.push(
      `export type ${singularPascalCase(enumName)} = (typeof ${enumName})[number];`
    );
  }
